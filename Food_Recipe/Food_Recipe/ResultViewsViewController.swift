...
class ResultViewsViewController: UIViewController, UISearchBarDelegate, UITableViewDataSource, UITableViewDelegate {
            ...
            if let imageURLString = recipe.image, let imageURL = URL(string: imageURLString) {
                Task {
                    do {
                        ...
                        DispatchQueue.main.async {
                            cell.imageView?.image = UIImage(systemName: "photo") // Error placeholder
                            cell.setNeedsLayout()
                        }
                    }
                }
            } else {
                cell.imageView?.image = UIImage(systemName: "photo") // No image URL placeholder
                cell.setNeedsLayout()
            }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "cell1", sender: selectedRecipe)
    }
